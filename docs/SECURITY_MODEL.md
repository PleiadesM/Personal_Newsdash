...
>
> Protected: the **contents** of private sections (and everything, in
> private mode) and your annotations' visibility.
...
- The AAD binds ciphertext to its section id: `news.enc.json` content can
  never be served to the reader as `papers`.
...
| Compromised Actions runner | Sees the secrets for that run — the same trust boundary as any CI system. Mitigation: scope-limited tokens, rotation habits |
...
- **Private-source capability URLs are credentials.** They exist only
  inside their GitHub Secret, are decoded in-process, and are never
...
- Build logs print no private counts, titles, or names — on a
...
- **Build-time (server → LLM endpoint).** Once per scheduled build, the
  ...
  bodies, never private-section content, never the passphrase. This is
...
- **Build-time (server → GDELT DOC API).** If Apropos-of-Nothing is enabled,
  ...
  terms. The request does **not** include private-section data, full
...
| A source capability URL or token leaked | Rotate it at the source (regenerate/reset), then update the corresponding GitHub Secret |
